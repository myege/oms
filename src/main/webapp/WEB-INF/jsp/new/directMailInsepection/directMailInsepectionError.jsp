...
<!-- 								id="search"><i class="fa fa-search"></i>&nbsp;搜索</a></li> -->
			...
			<div class="btn-group-sm" role="group">
				...
				<button type="button" id="order_bonded_btn_16" style="margin-top: 10px;" class="btn btn-success" onclick="updateAuditstatus0_mail()"><i class="fa fa-download"></i> 
				转为正常单
				</button>
			</div>
...
//转为正常单
function updateAuditstatus0_mail(){
		var rows =$("#bootstrap-table").bootstrapTable('getSelections');
		var length =rows.length;
		if(length==0){
			$.modal.alertWarning('请至少选择一个进行操作！');
		}else{
			$.modal.confirm('确定要修改这<font color=red>'+rows.length+'条订单吗?',function(){				
						var ids="";
						...
						layerMsg("修改中...");
						$.post('<%=path%>/orderMail/updateAuditstatus0.do',{ids:ids},
								function(ret){
							layer.close(layer.index);
									if(ret==1){
										layer.msg("修改成功", {
