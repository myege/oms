	...
	<div class="container-div">
		<div class="row">
							...
							<li>
		 						...
		 						<div class="input-group-btn"><!--不换行，与相邻元素内联，包含dropdown-->
			  						<select id="serachname" name="serachname" class="selectpicker show-tick form-control" data-max-options="3" data-live-search="false">
										<option value="text5">提单号</option>
										<option value="text4">流水号</option>
			...
			<div class="btn-group-sm hidden-xs" id="toolbar" role="group">
				...
				<a class="btn btn-warning" onclick="importcgd()"> <i class="fa fa-download"></i> 导入</a>
				<a style='color:blue' href='<%=path%>/xls/ijdcgd.xlsx'>下载采购单模板</a>
...
<script type="text/javascript">
	...
	**/
	...
	function importcgd() {
		var openUrl = "<%=path%>/import/importCgd.jsp";//弹出窗口的url
		var title = '采购单导入EXCEL';//弹出窗口的title
    		...
    		if(length==0){
    			...
    			$.modal.confirm('确定要删除这<font color=red>'+rows.length+'条记录吗?',function(){
    	       		layerMsg("删除中...");
    						...
    						$.post('<%=path%>/jdwl/deletecgd.do',{ids:ids},
    								function(ret){
    							layer.close(layer.index);
    									if(ret>0){
    										layer.msg("删除成功", {
