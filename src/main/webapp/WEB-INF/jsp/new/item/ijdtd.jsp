	...
	<div class="container-div">
		<div class="row">
							...
							<li>
		 						...
		 						<div class="input-group-btn"><!--不换行，与相邻元素内联，包含dropdown-->
			  						<select id="serachname" name="serachname" class="selectpicker show-tick form-control" data-max-options="3" data-live-search="false">
										<option value="text5">回执</option>
										<option value="text4">提单号</option>
										<option value="text3">采购单号</option>
									</select>
		  							</div>
			...
			<a class="btn btn-success" id="add" onclick="push()"> <i class="fa fa-plus"></i> 推送湾流</a>
				...
				<a class="btn btn-warning" onclick="importtd()"> <i class="fa fa-download"></i> 导入</a>
				<a style='color:blue' href='<%=path%>/xls/ijdtd.xlsx'>下载提单模板</a>
...
<script type="text/javascript">
	...
	**/
	...
	function importtd() {
		var openUrl = "<%=path%>/import/importTd.jsp";//弹出窗口的url
  		...
  		if(length==0){
  			...
  			$.modal.confirm('确定要删除这<font color=red>'+rows.length+'条记录吗?',function(){
  	       		layerMsg("删除中...");
  						...
  						$.post('<%=path%>/jdwl/deletetd.do',{ids:ids},
  								function(ret){
  							layer.close(layer.index);
  									if(ret>0){
  										layer.msg("删除成功", {
