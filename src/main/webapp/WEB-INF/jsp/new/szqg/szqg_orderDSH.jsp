...
<!-- 				</a> -->
				 ...
				 <a class="btn btn-warning" onclick="$.operate.importModel('50%','50%','<%=path%>/import/importSzqgOrder.jsp')"> <i class="fa fa-download"></i> 导入
				</a>
				<a style='color:blue' href='<%=path%>/xls/orderBonded.xlsx'>下载保税模板</a>
				
				
				<a class="btn btn-primary btn-edit" onclick="shpush();"> <i class="fa fa-download"></i>
					审核
				</a>
				
				<a class="btn btn-primary btn-edit" onclick="savekjTime();"> <i class="fa fa-download"></i>
				提单号导出
				</a>
				<!--  <button type="button" class="btn btn-success" style="margin-top: 10px;" 
					onclick="savekjTime()"> <i class="fa fa-plus-square"></i> 
					提单号导出
				</button> -->
			</div>
...
// 			$.operate.time("time4");
			...
			if(length==0){
				...
				$.modal.confirm('确定要推送这<font color=red>'+rows.length+'条记录吗?',function(){
		       	//	layerMsg("删除中...");
							...
							$.post('<%=path%>/szqgorder/shpush.do',{ids:ids},
									function(ret){
    ...
    function savekjTime(){
    	...
    	layer.open({
    	  	  ...
    	  	  type: 2,
    	  	  closeBtn: 2,
    	  	  content: '<%=path%>/szqgorder/tdhdc.do',
    	  	  btn: ['确认', '取消'],
    	  	  isOutAnim : true,
    	  	  maxmin  : true,
    	  	  anim : 2 ,
