$(function() {
	...
	$(".toolbar .category").each(function(i, a) {
		if ($(a).find("a.btn-primary").length == 0) $(a).find("a:eq(0)").addClass("btn-primary")
	});
	if ($(".toolbar .category").size() > 0) {
		var b = "<div class='btn-group mini'>";
		b += "<button class='btn btn-default dropdown-toggle' data-toggle='dropdown'>" + $(".toolbar .category a.btn-primary").html() + " <span class='caret'></span></button>";
		b += "<ul class='dropdown-menu dropdown-menu-right'>";
		$(".toolbar .category a").each(function(i, a) {
			b += "<li><a href='" + $(this).attr("href") + "'>" + $(this).html() + "</a></li>"
		});
		b += "</ul>";
		b += "</div>";
		$(".toolbar").append(b)
	}
	$(".toolbar .btnList .btn").each(function(i, a) {
		if ($(a).attr("href").indexOf("javascript:") == 0) $(a).addClass("disabled")
	});
	if ($(".toolbar .btnList").size() > 0) {
		var b = "<div class='mini'>";
		b += $(".toolbar .btnList").html();
		b += "</div>";
		$(".toolbar").append(b);
	...
	$("header nav ul li a:gt(0)").each(function(i, a) {
		if (c.indexOf($(a).attr("href").toLowerCase()) > 0) $(a).parent().addClass("active")
	});
	...
	$("section.content .words p").each(function(i, a) {
		if ($(a).css("text-align") == "center") $(a).css("text-align", "").attr("align", "center")
	});
	$("section.content .words img").each(function(i, a) {
		if ($(a).attr("border") == "1") $(a).css("border", "1px solid #ccc")
	});
...
function setArtlist() {
	...
	var a = $("header").outerWidth();
	var b = $(".toolbar .switch");
	if (b.css("display") == "none" || b.hasClass("glyphicon-th-list")) a = 0;
	var c = $(".artlist ul.items li p.left").outerWidth();
	var d = $(".artlist ul.items").css("margin-left").replace("px", "");
	var e = 100;
	if ($(".artlist ul.items li p.left").css("display") == "none") e = 0;
	else if ($(".dock").css("display") == "none") e = 30;
	$(".artlist ul.items li p.right").width($(window).width() - a - d - c - e)
}
function setArtshow() {
	...
	if (b.css("display") == "none" || b.hasClass("glyphicon-th-list")) a = 0;
	var c = 0;
	var d = $(".artshow .box").css("margin-left").replace("px", "");
	if ($(".artshow aside").width() > 0) {
		c = 75;
		d = d * 2
	}
	$(".artshow .box").width($(window).width() - a - d - $(".artshow aside").outerWidth() - c);
	var e = parseInt(($(window).height() - $(".artshow aside").outerHeight()) / 2);
	$(".artshow aside").css("top", e).addClass("active")
}
function setCaselist() {
	...
	var b = $(".toolbar .switch");
	if (b.css("display") == "none" || b.hasClass("glyphicon-th-list")) a = 0;
	$("section.caselist").width($(window).width() - a);
	var c = parseInt($(window).width() / $(".caselist ul.items li").width());
	var d = parseInt($(".caselist ul.items li").css("margin-left").replace("px", "")) + 2;
	var w = $(window).width() - $(".caselist ul.items li").width() * c - d * c * 2 + 5;
...
function setCaseshow() {
	...
	var b = $(".toolbar .switch");
	if (b.css("display") == "none" || b.hasClass("glyphicon-th-list")) a = 0;
	var w = $(window).width() - a;
	var c = 200;
	if ($(".dock").css("display") == "none") c = 100;
	$(".caseshow").width(w);
	if (w - c < 1000) $(".caseshow .demo").width(w - c).find("img").addClass("img-responsive")
}
function dockEvent() {
