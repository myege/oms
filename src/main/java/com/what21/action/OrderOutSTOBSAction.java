 ...
 @Controller
 @RequestMapping({"/orderOutSTOBS"})
 public class OrderOutSTOBSAction
   extends BaseAction
 {
   @Autowired
   private OrderOutSTOBSService orderOutSTOService;
   
   @RequestMapping({"/findAll"})
   @ResponseBody
   public DatagridResultInfo findAll(OrderOutSTOQueryVoBS orderOutSTOQueryVo, int rows, int page, HttpSession session) throws Exception {
   ...
   @ResponseBody
   public List<String> pushkouAn(int[] ids) throws Exception {
     return this.orderOutSTOService.pushkouAn(ids);
   }
