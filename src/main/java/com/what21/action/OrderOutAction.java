   ...
   extends BaseAction
 {
   ...
   @RequestMapping({"/find"})
   @ResponseBody
   public DatagridResultInfo find(int page, int rows, OrderOutQueryVo orderOutQueryVo, HttpSession session) throws Exception {
     ...
     System.out.println("准备时间：" + excTime + "s");
     int total = this.orderOutService.count(orderOutQueryVo).intValue();
     long endTime1 = System.currentTimeMillis();
     float excTime1 = (float)(endTime1 - startTime) / 1000.0F;
     System.out.println("查询全部：" + excTime1 + "s");
     List<OrderOut> list = this.orderOutService.find(orderOutQueryVo);
     long endTime2 = System.currentTimeMillis();
     float excTime2 = (float)(endTime2 - startTime) / 1000.0F;
     System.out.println("查询总数：" + excTime2 + "s");
     DatagridResultInfo datagridResultInfo = new DatagridResultInfo();
     datagridResultInfo.setRows(list);
     datagridResultInfo.setTotal(total);
     long endTime3 = System.currentTimeMillis();
     float excTime3 = (float)(endTime3 - startTime) / 1000.0F;
     System.out.println("方法完成：" + excTime3 + "s");
   ...
   @ResponseBody
   public DatagridResultInfo findSku(int page, int rows, OrderOutQueryVo orderOutQueryVo) throws Exception {
     int total = this.orderOutService.countSku(orderOutQueryVo).intValue();
     PageQuery pageQuery = new PageQuery(page, rows);
     orderOutQueryVo.setPageQuery(pageQuery);
     List<OrderOutSku> list = this.orderOutService.findSku(orderOutQueryVo);
     DatagridResultInfo datagridResultInfo = new DatagridResultInfo();
     datagridResultInfo.setTotal(total);
     datagridResultInfo.setRows(list);
     return datagridResultInfo;
   ...
   @ResponseBody
   public List<String> pushQd(int[] ids, HttpServletRequest request) throws Exception {
     String totalWayBill = request.getParameter("totalWayBill");
     return this.orderOutService.pushQd(ids, totalWayBill);
   }
   @RequestMapping({"/pushZsYd"})
   @ResponseBody
   public int pushZsYd(int[] ids) throws IOException, DocumentException {
     int message = this.orderOutService.pushZsYd(ids);
     return message;
   }
   
   @RequestMapping({"/pushSTO2"})
   @ResponseBody
   public List<String> pushSTO2(int[] ids, HttpServletRequest request) throws Exception {
     String totalWayBill = request.getParameter("totalWayBill");
     return this.orderOutService.pushSTO2(ids, totalWayBill);
   }
   ...
   public String updateSkuUI(Integer id, Model model) throws Exception {
     OrderOutSku orderOutSku = this.orderOutService.findSkuById(id);
     model.addAttribute("orderOutSku", orderOutSku);
     return "updateOutSku";
   }
   @RequestMapping({"/updateSku"})
   @ResponseBody
   public int updateSku(OrderOutSku orderOutSku) throws Exception {
     try {
   ...
   public OrderOutLog detail(Integer id) {
     OrderOut orderOut = this.orderOutService.findById(id);
   ...
   public void exportOrderOut(int[] ids, HttpServletRequest request, HttpServletResponse response) throws Exception {
     ...
     List<OrderOutForExport> list = null;
     if (ids.length != 0) {
       list = this.orderOutService.findExportByIds(ids);
     } else {
       OrderOutQueryVo orderOutQueryVo = (OrderOutQueryVo)request.getSession().getAttribute("orderOutQueryVo");
     ...
     cell.setCellStyle(cellStyle);
     int i = 1;
     for (OrderOutForExport orderOut : list) {
   ...
   public void exportOrderOu2t(int[] ids, HttpServletRequest request, HttpServletResponse response) throws Exception {
     ...
     for (int j = 0; j < list.size(); j++) {
       OrderOutForExport orderOut = list.get(j);
       if (j == list.size() - 1) {
         if (orderno == null) {
         ...
         OrderOutForExport orderOut2 = list.get(j + 1);
         ...
         if (!orderOut.getOrderno().equals(orderOut2.getOrderno())) {
           ...
           if (orderno == null) {
             ...
             !str.contains(orderOut.getGoodsname())) {
             str = String.valueOf(str) + "/" + orderOut.getGoodsname();
           } 
         } 
       }       
       ...
       cell = row.createCell(8);
       cell.setCellValue(orderOut.getGrossweight2());
       cell.setCellStyle(cellStyle);
       
       cell = row.createCell(9);
       cell.setCellValue(orderOut.getNetweight2());
       cell.setCellStyle(cellStyle);
       ...
       cell = row.createCell(13);
       cell.setCellValue(orderOut.getGoodsamount2());
       cell.setCellStyle(cellStyle);
       ...
       cell = row.createCell(15);
       cell.setCellValue(orderOut.getTotalprice2());
       cell.setCellStyle(cellStyle);
   ...
   @RequestMapping({"/pushWLD"})
   @ResponseBody
   public List<String> pushWLD(int[] ids, HttpServletRequest request) throws Exception {
     String totalWayBill = request.getParameter("totalWayBill");
     return this.orderOutService.pushWLD(ids, totalWayBill);
   }
   
   @RequestMapping({"/outDD"})
   public String outDD(HttpServletResponse resp, int[] id, int mesg, HttpSession session) throws Exception {
     ...
     String str = "";
     if (mesg == 505) {
       str = this.orderOutService.outYD(id, mesg, parseInt);
     } else if (mesg == 509) {
       str = this.orderOutService.outLJD(id, mesg, parseInt);
     } else if (mesg == 607) {
       str = this.orderOutService.outQDZFD3J(id, mesg, Integer.valueOf(parseInt));
     } else if (mesg == 605) {
       str = this.orderOutService.outCloseQd(id, mesg);
     } else if (mesg == 111) {
       str = this.orderOutService.outST(id, parseInt);
     } else {
       str = this.orderOutService.outDD3J(id, mesg, Integer.valueOf(parseInt));
     } 
   ...
   @RequestMapping({"/jxsh"})
   public String jxsh(HttpServletResponse resp, String id, HttpSession session) throws Exception {
     JSONObject result = new JSONObject();
     String str = "";
     List<Map<String, Object>> jxsh = this.orderOutService.jxsh(id);
     for (Map<String, Object> map : jxsh) {
       if (map.get("ba").toString().equals("0")) {
         str = String.valueOf(str) + "订单" + map.get("orderno") + "中" + map.get("o") + "不正确\r\n";
       }
     } 
     if (str == null || str.equals("")) {
       result.put("result", "success");
