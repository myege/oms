 ...
 @Controller
 @RequestMapping({"/orderOutRuleBS"})
 public class OrderOutRuleBSAction
   extends BaseAction
 {
   @Autowired
   ...
   public String updateUI(Integer id, Model model) {
     ...
     return "updateOrderOutRule1";
   }
   
   @RequestMapping({"/updateUI1"})
   public String updateUI1(Integer id, Model model) {
     OrderOutRule orderOutRule = this.orderOutRuleService.findById(id);
     model.addAttribute("orderOutRule", orderOutRule);
     return "updateOrderOutRule1";
   }
